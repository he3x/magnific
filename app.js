...
function renderVideoPreviews() {
    elements.videoPreview.innerHTML = state.videos.map((vid, index) => `
        <div class="preview-item">
            ...
            <div class="file-name" title="${vid.name}">${vid.name}</div>
...
async function generateMotion(imageFile, videoFile) {
    const imageUrl = await uploadFilePublic(imageFile);
    const videoUrl = await uploadFilePublic(videoFile);
    
    if (!imageUrl || !videoUrl) {
        throw new Error('Gagal mendapatkan URL publik untuk file');
    }
    
    const endpoint = CONFIG.ENDPOINTS[state.tier].generate;
    const payload = {
        image_url: imageUrl,
        video_url: videoUrl,
    ...
    const response = await fetch(endpoint, {
        ...
        headers: {
            ...
            'x-magnific-api-key': state.apiKey
        },
...
async function pollTaskStatus(taskId, progressElement) {
    const statusUrl = `${CONFIG.ENDPOINTS[state.tier].status}${taskId}`;
    ...
    for (let attempt = 1; attempt <= CONFIG.MAX_POLL_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (['COMPLETED', 'DONE', 'SUCCEEDED', 'SUCCESS'].includes(status)) {
                ...
                const errorMsg = data.message || data.error || 'Terjadi kesalahan di server';
            ...
            console.warn('Error saat cek status, retry...', error);
        }
    }
...
async function startGeneration() {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        try {
            ...
            const taskId = await generateMotion(task.image.file, task.video.file);
